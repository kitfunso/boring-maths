...
_Unverified, do not ship without a primary check:_ MAS car loan LTV caps (mas.gov.sg
...
_Unverified, do not ship without a primary check:_ the CPF valuation/withdrawal/accrued
...
_Unverified, do not ship without a primary check:_ SRS caps, the self-employed MediSave
age-by-income table (incomplete), the $80,000 overall relief cap, earned income relief,
and the ERS CPF LIFE payout figure.

_Two traps worth keeping even if we never build:_ Course Fees Relief died at YA2026 and
...
_One honesty flag:_ CPF Board does not publish a CPF LIFE payout formula for arbitrary
...
_Method caveat the lane stated plainly:_ no keyword volume tool was reachable, so "demand"
...
## 4.5 Decision: build it

**Keith's call, 2026-09-20: build the Singapore cluster.** It is built, and section 4.7
lists what shipped. The analysis below is kept unchanged because it is the honest reading
of the data and it is what the next review should be measured against.

### 4.5a What the evidence said

**The evidence argued against building.** Three independent lines converge:
...
The risk, stated once so the review has something to check against: this is the shape of
the UK cluster decision, and the UK cluster returned 0 clicks from 10,421 impressions.

### 4.5b Why it was built anyway

Keith directed it. Two things make that defensible against my own read:

- The research produced primary-sourced rate tables that are genuinely hard to assemble.
  Ten pages built on verified CPF, MOM, MOF and PUB figures cost one session, not a
  quarter, so the downside is bounded.
- A few of the pages are correct where incumbents are not. The Enhanced Retirement Sum
  became 4x BRS on 1 Jan 2025 and older sources still say 3x; Course Fees Relief died at
  YA2026 and live competitor calculators still offer it; the AWS page shows both proration
  methods because MOM mandates neither, where every competitor silently picks one. Being
  right where the incumbent is stale is the only lever a new entrant actually has.

---

## 4.7 What shipped

Ten calculators, registry 171 to 181, plus `/guides/best-singapore-calculators/` as the
cluster hub. Every rate carries its effective date on the page, because that is the one
thing the incumbents are bad at.

| slug                                      | what it computes                          | rate source                           |
| ----------------------------------------- | ----------------------------------------- | ------------------------------------- |
| `singapore-take-home-pay-calculator`      | employee CPF and net pay by age band      | CPF Board, from 1 Jan 2026            |
| `singapore-employer-cost-calculator`      | employer CPF plus Skills Development Levy | CPF Board and SDL rules               |
| `singapore-income-tax-calculator`         | resident progressive brackets             | IRAS, YA2026                          |
| `singapore-stamp-duty-calculator`         | BSD bands plus ABSD by status             | MOF, from 15 Feb 2023 and 27 Apr 2023 |
| `singapore-seller-stamp-duty-calculator`  | SSD, correct table by purchase date       | IRAS, two tables split at 4 Jul 2025  |
| `singapore-cpf-retirement-sum-calculator` | standing against BRS, FRS, ERS            | CPF Board, 2026 sums                  |
| `singapore-notice-period-calculator`      | notice, salary in lieu, leave encashment  | MOM                                   |
| `singapore-aws-bonus-calculator`          | AWS proration, both methods               | MOM (no mandated formula)             |
| `singapore-gst-calculator`                | GST at 9% plus import landed cost         | IRAS, from 1 Jan 2024                 |
| `singapore-utility-bill-calculator`       | SP electricity and tiered PUB water       | SP Group Q3 2026, PUB from 1 Apr 2025 |

Three deliberate correctness choices the incumbents get wrong, and the reason the build is
worth more than its click forecast:

- The ERS is **4x BRS** ($440,800), not 3x. It changed on 1 Jan 2025 and stale sources
  still carry the old multiple.
- The SSD page **requires the purchase date**, because the table split on 4 Jul 2025 and
  using the wrong one is wrong by tens of thousands of dollars.
- The CPF page **refuses to output a CPF LIFE monthly payout**. CPF Board publishes no
  formula for arbitrary balances, so every precise figure on a competitor site is an
  interpolation presented as a fact.

**Known rate debt, from 4.4:** foreign worker levy by sector, the GPL maternity and
paternity caps, Part IV overtime thresholds and the maid levy concessions were all
secondary-sourced only, so nothing was built on them. Any second wave needs a primary
fetch first.

**One gap in what did ship.** The YA2026 income tax brackets could not be confirmed
first-party: the IRAS rate page is a JavaScript app and two fetches returned navigation
only. They are confirmed against two independent secondary sources that agree with each
other and with the expected structure. That is weaker than every other rate in the cluster,
and it is the first thing to re-check against IRAS directly.
...
The cluster is built, so the staging question is closed. What remains is the measurement,
and it is written down here **before** the data arrives so it cannot be renegotiated after.

**Measure at 8 weeks from the deploy date.** Pull the 28-day GSC export and cut it to the
ten `singapore-*` pages.

- **PASS** if the cluster reaches a weighted average position under 25 **and** produces any
  clicks at all. Then build the second wave from the 4.4 rate sheets.
- **FAIL** if it lands at position 50+ with zero clicks. That is exactly what the UK cluster
  did, and it would make Singapore the fourth failed country cluster. Then stop, and no
  fifth one gets built without a mechanism that explains why the first four failed.
- Anything between those two is a partial: keep the pages, build no more.

The comparison that matters is not "did it get traffic" but "did it beat the UK cluster at
the same age". UK cluster at 28 days is the baseline, and section 3 has the numbers.

**One thing to watch that is not in the gate:** if these pages rank while the generic pages
do not, the lesson is about page quality and rate freshness, not about Singapore. Check
whether the SG pages outrank our own generic pages before concluding the country thesis
worked.

### Constraints that bind this work

- `CLAUDE.md` rule 5: no calculators added just for SEO. Each must solve a real problem.
  Every page in the cluster computes a published government rate against a user's own
  numbers, which passes that test on its merits.
- `tests/seo/registry-guard.test.ts` was pinned at 171 and bumped to 181 on 2026-09-20 on
  Keith's direction, which is the sign-off the guard comment asks for.
